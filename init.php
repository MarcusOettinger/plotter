...
/*
This file contains a security check for the formulae,
transforms them into a PHP-readable form and defines
them. Finally the function value for a specific
input value is calculated by the function 'graph()'.
The first part is to turn ^ into the pow function.

It is used both for drawing a graph and calculating a 
single value and is included by graph.php and by single.php.
*/
...
// add multiplication sign where needed, e.g. 3x -> 3*x
...
/* security check of the formulae
This is done, because the formulae are executed via 'eval', which 
could be very dangerous, if we don't prevent malicious code. So we
only allow functions and characters, which the program understands 
and which can cause no harm.
The test deletes everything that is allowed. If something remains,
the programm stops.*/
for($i=0;$i<3;$i++) {
...
/*
Here now the functions not included in PHP are defined 
(of course those already included, like 'sin()', we 
don't have to define).

Their sequence is not very obvious, they are ordered 
sometimes in the way I programmed them, sometimes not.
Feel free to reorder, here it will do no harm :-)
*/
...
// square root with NULL as return value for x<0
function sqr($a) { return $a<0?NULL:sqrt($a); }

// natural logarithm with NULL as return value for x<=0
function nlog($a) { return $a<0?NULL:log($a); }

// decadic logarithm with NULL as return value for x<=0
